...
/* sidebar custom */
include (TEMPLATEPATH . '/libs/widget.php' );	// Framework Init

// Register widgetized areas
if ( ! function_exists( 'widgets_c_init' ) ) {
	function widgets_c_init() {
	    if ( ! function_exists( 'register_sidebars' ) )
	        return;


	    register_sidebar( array( 'name' => __( 'sidebar custom', 'woothemes' ), 'id' => 'sidebar_c', 'description' => __( 'The default primary sidebar for your website, used in two or three-column layouts.', 'woothemes' ), 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3>', 'after_title' => '</h3>' ) );

	}
}

add_action( 'init', 'widgets_c_init' );


// Function that outputs the contents of the dashboard widget
function dashboard_widget_function() {
    $pages = get_option('pages_expired');
    ...
    foreach ($pages as $page) {
        $html .= '<td>'.get_the_title($page).'</td><td><a href="'.get_edit_post_link($page).'">Editar</td>';
    }
...
function add_dashboard_widgets() {
    wp_add_dashboard_widget('dashboard_expired', 'Páginas que estão expiradas', 'dashboard_widget_function');
    ...
    $expired_widget_backup = array( 'dashboard_expired' => $normal_dashboard['dashboard_expired'] );
    unset( $normal_dashboard['dashboard_expired'] );

    // Merge the two arrays together so our widget is at the beginning

    $sorted_dashboard = array_merge( $expired_widget_backup, $normal_dashboard );
...
// wp_register_style('font-awesome',CSS_URL.'font-awesome/css/font-awesome.min.css');
